...
model = dict(
    ...
    img_neck=dict(
        ...
        num_outs=_num_levels_, # 3 scale features, one empty or sth 
        relu_before_extra_convs=True),
    bev_constructor=dict(
        type='BEVFormerConstructer',
        num_feature_levels=_num_levels_, 
        num_cams=num_cams, # subset_A: 7, subset_B: 6 
        ...
        bev_h=bev_h_, # 100
        bev_w=bev_w_, # 200 
        rotate_center=[bev_h_//2, bev_w_//2], # for what 
        encoder=dict(
            type='BEVFormerEncoder', # 6 layers encoder  
            num_layers=6, # 
            pc_range=point_cloud_range, # 
            num_points_in_pillar=4, # why 5 points pillar, 
                ...
                attn_cfgs=[
                    ...
                    dict(
                        ...
                        deformable_attention=dict(
                            ...
                            embed_dims=_dim_,
                            num_points=8, # 8 points 
                            num_levels=_num_levels_) # 4 levels 
                    )
                ],
                ffn_cfgs=_ffn_cfg_,
        ...
        positional_encoding=dict(
            type='LearnedPositionalEncoding', # learnable params, positional encoding 
            num_feats=_pos_dim_,
    ...
    lane_head=dict(
        type='LaneSegHead',
        num_classes=1, # 1 classes only 
        num_lane_type_classes=3, # lane types
        in_channels=_dim_, # 
        num_query=200,
        bev_h=bev_h_,
        bev_w=bev_w_,
    ...
    area_head=dict(
        ...
        dir_interval=1,
        num_classes=2, # ped_crossing, road boundary 
        in_channels=_dim_,
        sync_cls_avg_factor=True,
        with_box_refine=True,
        as_two_stage=False,
        ...
        transformer=dict(
            type='MapTRTransformerDecoderOnly', # only use the decoder from MapTR, 
            embed_dims=_dim_, # 256
            pts_dim=pts_dim, # 3 
            decoder=dict(
                type='MapTRDecoder', # 
                num_layers=6, # 6 is the standard number 
                return_intermediate=True,
                pts_dim=pts_dim, # 3 - x,y,z 
                transformerlayers=dict(
                    type='DetrTransformerDecoderLayer',
                    attn_cfgs=[
                        dict(
                            ...
                            dropout=0.1),
                         dict(
                            type='CustomMSDeformableAttention', # is it the inter-ins and intra-ins attention 
                            embed_dims=_dim_, # from MapTR 
                            num_levels=1),
                    ],
                    feedforward_channels=_ffn_dim_,
                    ffn_dropout=0.1,
    ...
    train_cfg=dict(
        bbox=dict(
            assigner=dict(
                type='HungarianAssigner', # bipartite matching to find the optimal assigment 
...
train_pipeline = [
    # load, load annotations, crop front view, random scale, normalize, pad, gridmask, lanesegment parameterize, generate lanesegment mask, custom format bundle 3D lane, custom collect 3d 
