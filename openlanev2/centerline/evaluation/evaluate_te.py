...
def evaluate(ground_truth, predictions, verbose=True):
    ...
    if 'lane_centerline' in predictions and len(predictions['lane_centerline']) > 0:
    ...
    # if 'traffic_element' in predictions and len(predictions['traffic_element']) > 0:
    ...
    if 'topology_lclc' in predictions and len(predictions['topology_lclc']) > 0:
        metrics['OpenLane-V2 Score']['TOP_ll'] = _mAP_topology_lclc(gts, preds, THRESHOLDS_FRECHET)
    if 'topology_lcte' in predictions and len(predictions['topology_lcte']) > 0:
